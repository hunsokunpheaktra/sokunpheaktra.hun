...
- (void)getDeleted:(NSString *)sObjectType fromDate:(NSDate *)startDate toDate:(NSDate *)endDate target:(id)target selector:(SEL)selector context:(id)context
{
    ...
    if([startDate laterDate:endDate] || [startDate isEqualToDate:endDate]){
        endDate = [endDate dateByAddingTimeInterval:60];
    }
    
    NSLog(@"Start Date : %@",startDate);
    NSLog(@"End Date : %@",startDate);
    /*
    ZKEnvelope *env = [[[ZKPartnerEnvelope alloc] initWithSessionId:sessionId updateMru:self.updatesMostRecentlyUsed clientId:clientId] autorelease];
    ...
    [getDeletedElement addChildElement:[FDCMessageElement elementWithName:@"startDate" value:[DatetimeHelper serverDateTime:startDate]]];
    [getDeletedElement addChildElement:[FDCMessageElement elementWithName:@"endDate" value:[DatetimeHelper serverDateTime:endDate]]];
