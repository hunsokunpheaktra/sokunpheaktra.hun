...
- (id)initWithEntity:(NSString *)newEntity listener:(NSObject<SyncListener>*) newListener level:(int)newLevel{
...
- (NSString*)getCriteria:(Item*)item{
    ...
    if ([entity isEqualToString:@"Case"] || [entity isEqualToString:@"Task"] || [entity isEqualToString:@"Event"]){
        subOrName = @"Subject";
    }
...
- (NSString*) getQueryContents{
    ...
    NSString* userId = [PropertyManager read:@"CurrentUserId"];
    ...
    if ([self.entity isEqualToString:@"Survey__c"]) {
        [querystring appendString:@" WHERE Date_Time_Scheduled__c = THIS_WEEK "]; 
        if (userId && userId.length > 0) {
            [querystring appendString:@" AND OwnerId = "];
            [querystring appendString:[NSString stringWithFormat:@"'%@'",userId]];
        }
        
    }else if ([self.entity isEqualToString:@"Event"]) {
        [querystring appendString:@" WHERE StartDateTime = THIS_WEEK "];
        if (userId && userId.length > 0) {
            [querystring appendString:@" AND OwnerId = "];
            [querystring appendString:[NSString stringWithFormat:@"'%@'",userId]];
        }
        
    }else if ([self.entity isEqualToString:@"User"] || [self.entity isEqualToString:@"RecordType"]) {
...
- (NSString*) getIdsString:(NSArray*)listItem {
    ...
    for (Item* ref in [self getOneToOneRelation]){
        ...
        for (NSString *name in referenceEntities) {
            ...
            for (Item *item in listItem) {
                if ([item.fields objectForKey:[ref.fields objectForKey:@"name"]] != (id)kCFNull) {
                    [idss appendString:[NSString stringWithFormat:@"'%@'",[item.fields objectForKey:[ref.fields objectForKey:@"name"]]]];
                    [idss appendString:@","];
                }    
            }
