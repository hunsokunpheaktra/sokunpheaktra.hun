...
+ (void)initTable {
    ...
    [database createIndex:FIELDSINFO_ENTITY column:@"entity" unique:false];
    [database createIndex:FIELDSINFO_ENTITY columns:[NSArray arrayWithObjects:@"entity",@"name", nil] unique:false];
}
...
+ (NSArray *) getReferencesTo: (NSString*) entity{
    ...
    NSArray* listField = [FieldInfoManager list:filter];
    NSMutableArray* listReference = [[NSMutableArray alloc] init];
    for (Item* objField in listField) {
        [listReference addObject:objField];
    }
