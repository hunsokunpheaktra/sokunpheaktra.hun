...
function isValid(workbook) {
  ...
  if(sheet['E1'] == undefined || !_.isString(sheet['E1'].v) || sheet['E1'].v.trim() != 'Retail Account')       return false;
  if(sheet['F1'] == undefined || !_.isString(sheet['F1'].v) || sheet['F1'].v.trim() != 'Retailer NAme')        return false;
...
function parseProduct(sheet, order) {
  ...
  flats.forEach(function(item) {
    ...
    if((products[item.productCode].stores[item.storeNo])){
    	var oldData = products[item.productCode].stores[item.storeNo];
    	oldData.quantity += item.quantity;
    	oldData.lampsPerSku += item.packQty;
    	oldData.totalPrice += item.totalIncentitive;
    }else{
